...
export async function navigateWebPage(date, url) {
    ...
    for(let i = 0 ; i <  market.length; i++){
        for(let j  = 0 ;j < market[i].data.dataNames.length; j++){
            ...
            if(name !== undefined){
              ...
              const finalName = setName(name)
              playerOdd.push({market: market[i].market,finalName,line,overOdd,underOdd})
            }
        }
    }
    return playerOdd
